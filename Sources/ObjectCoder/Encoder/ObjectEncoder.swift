...
public class ObjectEncoder<Encoded>: Encoder {
	...
	public enum KeyEncodingStrategy {
		...
		func transform(codingKey: CodingKey, codingPath: [CodingKey]) -> CodingKey {
			switch self {
			case .useDefaultKeys:
				return codingKey
			case .convertToSnakeCase:
				let newKeyString = _convertToSnakeCase(codingKey.stringValue)
				return AnyCodingKey(stringValue: newKeyString, intValue: codingKey.intValue)
			case .custom(let converter):
				return converter(codingPath + [codingKey])
			}
		}
	...
	public struct Options {
		...
		public init(keyEncodingStrategy: KeyEncodingStrategy, userInfo: [CodingUserInfoKey: Any]) {
			self.keyEncodingStrategy = keyEncodingStrategy
			self.userInfo = userInfo
		}
	}
	
	let options: ObjectEncoder.Options
	...
	public init(options: ObjectEncoder.Options, boxer: Boxer) {
